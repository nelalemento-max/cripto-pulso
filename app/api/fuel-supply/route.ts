...
export async function GET(request:NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({stations:enrichedStations,trend:trendRows??[],daily:[...dailyMap.values()],sourceTime:payload.server_time??observedAt,observedAt,source:"ANH Abastecimiento",methodology:{official:"Litros, venta activa, despacho, ubicación y hora de actualización son reportados por ANH.",estimated:"Capacidad, porcentaje, ventas, recargas y autonomía son estimaciones de CriptoPulso calculadas por variación entre lecturas.",cadenceMinutes:15}},{headers:{"Cache-Control":"public, s-maxage=60, stale-while-revalidate=180"}});
  } catch(error){console.error("ANH liters fetch failed",error); return NextResponse.json({error:"La fuente ANH no respondió o el historial todavía no está preparado. Intenta nuevamente en unos minutos."},{status:502})}
}
